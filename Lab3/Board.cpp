...
Board::Board(int n, const vector<char>& colors) : n(n), colors(colors) {
...
bool Board::swapColor(int x1, int y1, int x2, int y2) {
...
void Board::bombBonus(int x, int y) {
    board[x][y] = '*';
    int i = 0;
    while (i < 4) {
        int randX = rand() % n;
...
void Board::recolorBonus(int x, int y) {
...
void Board::applyBonus(int x, int y) {
    vector<pair<int, int>> cells;
    for (int i = max(0, x - 3); i <= min(n - 1, x + 3); i++) {
        for (int j = max(0, y - 3); j <= min(n - 1, y + 3); j++) {
            if (board[i][j] != ' ' && abs(i - x) + abs(j - y) > 1 && abs(i - x) + abs(j - y) <= 3) {
...
bool Board::FindAndRemoveMatches(bool bonus) {
